...
| `--output-format` | `TLS_COMPLIANCE_OUTPUT_FORMAT` | Output format: `csv`, `json`, `yaml`, `junit`, `markdown`, `html`, `sarif` |
...
|----------|---------|---------|
| `SCAN_FORMAT` | `junit` | `csv`, `json`, `yaml`, `junit`, `markdown`, `html`, `sarif` |
...
- name: Export SARIF
  if: always()
  run: kubectl tlsreport sarif > tls-compliance.sarif

- name: Upload SARIF
  if: always()
  uses: github/codeql-action/upload-sarif@v3
  with:
    sarif_file: tls-compliance.sarif
```
