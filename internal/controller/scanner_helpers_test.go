...
func TestNestedStringField(t *testing.T) {
	...
	t.Run("top-level nested field exists", func(t *testing.T) {
		got, found := nestedStringField(obj, "status", "certificationStatus")
		if !found || got != "Certified" {
			t.Errorf("nestedStringField = (%q, %v), want (Certified, true)", got, found)
		}
	})

	t.Run("deeply nested field exists", func(t *testing.T) {
		got, found := nestedStringField(obj, "status", "pyxisData", "healthIndex")
		if !found || got != "A" {
			t.Errorf("nestedStringField = (%q, %v), want (A, true)", got, found)
		}
	})

	t.Run("field missing at leaf", func(t *testing.T) {
		got, found := nestedStringField(obj, "status", "missing")
		if found || got != "" {
			t.Errorf("nestedStringField = (%q, %v), want (, false)", got, found)
		}
	})

	t.Run("intermediate map missing", func(t *testing.T) {
		got, found := nestedStringField(obj, "nonexistent", "field")
		if found || got != "" {
			t.Errorf("nestedStringField = (%q, %v), want (, false)", got, found)
		}
	})
	...
	t.Run("intermediate value not a map", func(t *testing.T) {
		got, found := nestedStringField(obj, "status", "certificationStatus", "subfield")
		if found || got != "" {
			t.Errorf("nestedStringField = (%q, %v), want (, false)", got, found)
		}
	})
