...
func TestNestedStringField(t *testing.T) {
	...
	t.Run("top-level nested field exists", func(t *testing.T) {
		got, found, err := nestedStringField(obj, "status", "certificationStatus")
		if err != nil || !found || got != "Certified" {
			t.Errorf("nestedStringField = (%q, %v, %v), want (Certified, true, nil)", got, found, err)
		}
	})

	t.Run("deeply nested field exists", func(t *testing.T) {
		got, found, err := nestedStringField(obj, "status", "pyxisData", "healthIndex")
		if err != nil || !found || got != "A" {
			t.Errorf("nestedStringField = (%q, %v, %v), want (A, true, nil)", got, found, err)
		}
	})

	t.Run("field missing at leaf", func(t *testing.T) {
		got, found, err := nestedStringField(obj, "status", "missing")
		if err != nil || found || got != "" {
			t.Errorf("nestedStringField = (%q, %v, %v), want (, false, nil)", got, found, err)
		}
	})

	t.Run("intermediate map missing", func(t *testing.T) {
		got, found, err := nestedStringField(obj, "nonexistent", "field")
		if err != nil || found || got != "" {
			t.Errorf("nestedStringField = (%q, %v, %v), want (, false, nil)", got, found, err)
		}
	})
	...
	t.Run("intermediate value not a map", func(t *testing.T) {
		got, found, err := nestedStringField(obj, "status", "certificationStatus", "subfield")
		if err != nil || found || got != "" {
			t.Errorf("nestedStringField = (%q, %v, %v), want (, false, nil)", got, found, err)
		}
	})
...
func TestNestedInt64Field(t *testing.T) {
	...
	t.Run("int64 value", func(t *testing.T) {
		got, found, err := nestedInt64Field(obj, "status", "daysUntilEol")
		if err != nil || !found || got != 180 {
			t.Errorf("nestedInt64Field = (%d, %v, %v), want (180, true, nil)", got, found, err)
		}
	})

	t.Run("float64 coerced to int64", func(t *testing.T) {
		got, found, err := nestedInt64Field(obj, "status", "pyxisData", "vulnerabilities", "critical")
		if err != nil || !found || got != 3 {
			t.Errorf("nestedInt64Field = (%d, %v, %v), want (3, true, nil)", got, found, err)
		}
	})

	t.Run("int coerced to int64", func(t *testing.T) {
		got, found, err := nestedInt64Field(obj, "status", "pyxisData", "countAsInt")
		if err != nil || !found || got != 7 {
			t.Errorf("nestedInt64Field = (%d, %v, %v), want (7, true, nil)", got, found, err)
		}
	})

	t.Run("field missing at leaf", func(t *testing.T) {
		got, found, err := nestedInt64Field(obj, "status", "missing")
		if err != nil || found || got != 0 {
			t.Errorf("nestedInt64Field = (%d, %v, %v), want (0, false, nil)", got, found, err)
		}
	})

	t.Run("intermediate map missing", func(t *testing.T) {
		got, found, err := nestedInt64Field(obj, "nonexistent", "field")
		if err != nil || found || got != 0 {
			t.Errorf("nestedInt64Field = (%d, %v, %v), want (0, false, nil)", got, found, err)
		}
	})
	...
	t.Run("value found but wrong type", func(t *testing.T) {
		...
		got, found, err := nestedInt64Field(wrongType, "val")
		if err != nil || found || got != 0 {
			t.Errorf("nestedInt64Field (wrong type) = (%d, %v, %v), want (0, false, nil)", got, found, err)
		}
	})
}
...
func newTestPod(name, namespace, container, imageID string) *corev1.Pod {
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: namespace},
		Status: corev1.PodStatus{
			ContainerStatuses: []corev1.ContainerStatus{
				{Name: container, ImageID: imageID},
			},
		},
	}
}

func newTestTLSReport(name string) *securityv1alpha1.TLSComplianceReport {
...
func newTestICIUnstructured(name, digestLabelValue string, status map[string]interface{}) *unstructured.Unstructured {
		...
		Object: map[string]interface{}{
			...
			"metadata": map[string]interface{}{
				...
				"labels": map[string]interface{}{
					iciDigestLabel: digestLabelValue,
				},
			},
			"status": status,
		},
...
func TestEnrichWithImageCertInfo_SkipsContainersWithEmptyImageID(t *testing.T) {
	...
	cr := newTestTLSReport("test-report")
...
func TestEnrichWithImageCertInfo_ListErrorNonMatch(t *testing.T) {
	pod := newTestPod("test-pod", "default", "app", testEnrichImageID)
	cr := newTestTLSReport("test-report")
...
func TestEnrichWithImageCertInfo_StatusUpdateError(t *testing.T) {
	pod := newTestPod("test-pod", "default", "app", testEnrichImageID)
	cr := newTestTLSReport("test-report")
	ici := newTestICIUnstructured("test-ici", testEnrichDigestLbl, map[string]interface{}{
		"certificationStatus": "Certified",
	})
	r := newEnrichReconciler(t, []client.Object{pod, cr}, &interceptor.Funcs{
		List: func(ctx context.Context, cl client.WithWatch, list client.ObjectList, opts ...client.ListOption) error {
