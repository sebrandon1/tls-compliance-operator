...
func TestEnumerateCiphers_IterationCap(t *testing.T) {
	cert, _ := generateTestCert(t)
	host, port, cleanup := startTLSServerWithCipherSuites(t, cert, tls.VersionTLS12, []uint16{
		tls.TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256,
		tls.TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384,
	...
	suites := result.CipherSuites["TLS 1.2"]
	if len(suites) > maxCipherIterations+1 {
		t.Errorf("cipher enumeration exceeded cap: got %d suites, max allowed %d",
			len(suites), maxCipherIterations+1)
	}
}

func TestEnumerateCiphers_ContextCancellation(t *testing.T) {
	cert, _ := generateTestCert(t)
	host, port, cleanup := startTLSServerWithCipherSuites(t, cert, tls.VersionTLS12, []uint16{
		tls.TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256,
		tls.TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384,
	...
	checker := NewTLSChecker(2 * time.Second)
	_, err := checker.CheckEndpoint(ctx, host, port)
	if err == nil {
